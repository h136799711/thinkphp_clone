...
final class Sms {
	...
	public function sendSms($token, $content='',$mobile='', $send_time='', $charset='utf-8',$id_code = '') {
		...
		if ($thisUser['smscount']<1){
			...
			if($thisUser['smscount'] <= 3){
				$content = '您的短信服务仅剩'.$thisUser['smscount'].'条了！'.$content;
			}
			$data = array(
			'topdomain' => C('server_topdomain'),
			'auth_key' => 'goorayehebidu',
			'token' => $token,
			'content' => $content,
			'mobile'=>$mobile,
			);
			...
			//短信发送地址
			$smsapi_senturl = 'http://api.gooraye.net/index.php/Api/Sms/send';

			$return = Sms::curl_post($smsapi_senturl, $post);
			$this->statuscode = $return->errcode;
			//增加到本地数据库
